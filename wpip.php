...
function wpip_activate() {
	...
	if ( !class_exists(ZipArchive) ){
		wp_die('Error: ZipArchive is not installed on this server.');
	}

	if ( !ini_get('allow_url_fopen') ) {
		wp_die('Error: allow_url_fopen needs to set "on" for WP Install Profiles to function correctly.');
	}
...
<form method="post" action="admin.php?page=installation_profiles" id="profileForm">
		...
		<p><strong>Plugins</strong> <em>(one per line)</em>: <span><a style="font-size:10px;position:relative;top:-2px;margin-left:45px" href="#" id="helpTrigger">How to add plugins</a></span><br/>
			<textarea name="pluginNames" id="pluginNames" style="width:70%;height:200px"><?php print esc_textarea($defaultLines); ?></textarea>
		</p>
